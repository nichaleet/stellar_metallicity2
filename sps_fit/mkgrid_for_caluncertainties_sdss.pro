...
common get_sps_alpha, element
   ...
   ;setting of SDSS spectra
   sdss = mrdfits('/scr2/nichal/workspace4/sps_fit/data/gallazzi_allmass2/sps_fit01.fits.gz',1,/silent)
   good = where(sdss.logmstar gt 0., cgood)
   sdss = sdss(good)   
   redshift = mean(sdss.zfit)

   obslambda = sdss[0].lambda
   dlam = sdss[0].dlam  ;already sigma, not fwhm
   restlambda = obslambda/(1.+redshift)
   nwl = n_elements(obslambda)
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;Mg region
   ...
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ;wavelength range
   w = where(obslambda/(1.+redshift) lt 3650. or obslambda/(1.+redshift) gt 6200.,c)
   if c gt 0 then mask[w]=0
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   mask[0:4] = 0
   mask[nwl-5:nwl-1] = 0
...
pro mkgrid_for_caluncertainties_sdss
   ...
   for ia = 0,99 do begin
     for iap=0,40 do begin
      ...
      outfile = 'sspdegen/sspdegen_sdss/age'+strtrim(string(ia,format='(I02)'),2)+'_sspdegen_gridspec_sdss'+'.fits'
