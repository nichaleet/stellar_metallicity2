...
pro sps_fit::fitomg, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    pi[0].limits = [-0.6,0.] ;this is just for initial parameters
    pi[1].limits = [min(spsage),(galage(znow,1000)/1.e9)<max(spsage)]
  ...
  ;  pi[0].limits[1]=0.0
    ...
    openw,long(copynum),'/scr2/nichal/workspace4/sps_fit/logsps/sps_fit_cl0024'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
...
;    endif else pi[2].limits = [40.,400.]; [30.,200.]
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    openw,long(copynum),'/scr2/nichal/workspace4/sps_fit/logsps/sps_fit_cl0024_glorious'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
    printf,long(copynum), 'prior range:',pi.limits
...
pro sps_fit::redraw
    ...
    case mode of
	    ...
	    oplot, science.lambda/(1.+znow),(science.contdivivar)^(-0.5)/science.spscont,color=fsc_color('blue')
