function make_spec,grid_feh,grid_age,grid_alpha=grid_alpha,full=full,short=short
...
pro mkgrid_for_caluncertainties_cl1604
   ...
   dirshort ='sspdegen/sspdegen_cl1604_short/' 
   dirfull = 'sspdegen/sspdegen_cl1604_full/'
   if ~file_test(dirshort,/directory) then file_mkdir,dirshort
   if ~file_test(dirfull,/directory) then file_mkdir,dirfull
   for ia = 0,99 do begin
      for iap=0,40 do begin
         spec_arr = make_spec(grid_feh,grid_age(ia),grid_alpha=[grid_alpha(iap)],/short)
         outfile = dirshort+'age'+strtrim(string(ia,format='(I02)'),2)+'_sspdegen_gridspec_shortwl'+'.fits'
         ...
         spec_arr = make_spec(grid_feh,grid_age(ia),grid_alpha=[grid_alpha(iap)],/full)
         outfile = dirfull+'age'+strtrim(string(ia,format='(I02)'),2)+'_sspdegen_gridspec_fullwl'+'.fits'
         if iap eq 0 then print, 'writing '+outfile
         if iap eq 0 then mwrfits,spec_arr,outfile,/silent,/create else mwrfits,spec_arr,outfile,/silent
         if spec_arr[0].alpha eq -999 then stop
      endfor
