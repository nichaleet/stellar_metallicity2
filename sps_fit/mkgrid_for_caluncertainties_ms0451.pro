...
common get_sps, dlam, dataivar, datalam, wonfit, contmask, normalize,rest
   ...
   restlambda = lambdaobs/(1.+redshift)
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   for i=0,cemlines-1 do begin
       w = where(obslambda/(1d + sample.z) ge linestart[wem[i]] and $
           obslambda/(1d + sample.z) le lineend[wem[i]], c)
  ...
  ;Mg region
   if ~keyword_set(alpha) then begin
      ...
      for i=0,n_elements(use_indices)-1 do begin
         ...
         w = where(obslambda/(1.+sample.z) gt indstart(indnow[0]) and $
             obslambda/(1.+sample.z) lt indend(indnow[0]),cw)
         if cw gt 0 then mask[w]=0
      endfor
   endif
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ;wavelength range
   w = where(obslambda/(1.+sample.z) lt 3650. or obslambda/(1.+sample.z) gt 6000.,c)
   if c gt 0 then mask[w]=0
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
