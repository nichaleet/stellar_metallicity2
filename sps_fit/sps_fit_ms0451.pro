...
pro sps_fit::fitomg, science, noredraw=noredraw, nostatusbar=nostatusbar
  ...
  ;  pi[0].limits[1]=0.3
    ...
    pi.parname = ['    Z', '  age', 'vdisp','redshift','Mg','N']
    ...
    openw,long(copynum),'/scr2/nichal/workspace4/sps_fit/logsps/sps_fit_ms0451'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    ...
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum), '  i Z/Z_sun   age sigma_v  redhift    Mg    N   chi^2  DOF'
    printf,long(copynum), '--- ------- ----- ------- --------- --------- -------- ---- -----  ------'
;;things to keep during the while loop
    bestchisq = 9999.
   ...
   ;;while abs(zdiff) gt 0.001 or abs(agediff) gt 0.001 or abs(vdispdiff) gt 0.001 or abs(redshfdiff) gt 0.001 and nloop le maxnloop do begin
    while nloop lt maxnloop do begin
        ...
        rest =0
        if nloop eq 0 then normalize =1 else normalize = 0
        pars = mpfitfun('get_sps_alpha_obs', xmp, ymp, dymp, parinfo=pi, /nocatch, bestnorm=bestnorm, dof=dof, perror=perror, ftol=1d-10, gtol=1d-10, xtol=1d-10, covar=covar, nprint=500, status=status, yfit=ympfit, iterproc='sps_iterproc_omg')
...
pro sps_fit::fit_glorious, science, noredraw=noredraw, nostatusbar=nostatusbar
   ...
   ; pi[1].limits=[2,3.]
    ...
    pi[4].limits =[-0.3,0.5]  ;Mg
    pi[5].limits = [-0.3,0.8] ;N
...
;    endif else pi[2].limits = [40.,400.]; [30.,200.]
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    openw,long(copynum),'/scr2/nichal/workspace4/sps_fit/logsps/sps_fit_ms0451_glorious'+copynum+'.log',/append
    printf,long(copynum), '* * * * * * * * * * * * * * * * * * * *'
    printf,long(copynum),systime()
    printf,long(copynum), strtrim(science.objname, 2)+'  ('+strtrim(string(self.i+1, format='(I3)'), 2)+' / '+strtrim(string(self.nspec, format='(I3)'), 2)+')'
    printf,long(copynum), 'prior range:',pi.limits
...
pro sps_fit::fit_all,alpha=alpha,omg=omg,glorious=glorious
    common mask_in, mask_in, copynum
    widget_control, widget_info(self.base, find_by_uname='keepoldfit'), get_value=keepoldfit
  ...
  for nwalker=0,3 do begin
	...
	;if science.good eq 0 and science.goodfit eq 0 then continue
        if ~keyword_set(alpha) and ~keyword_set(omg) and ~keyword_Set(glorious) $
           then self->fit, science
...
pro sps_fit::getscience, files=files
    ...
    if ~file_test(sciencefits) then begin
        ...
        for i=0,nspec-1 do begin
            ...
            science.objname = data.objname
            science.mask = data.mask
            science.slit = data.slit

            science.ra = data.ra
            science.dec = data.dec
            ...
            ;photometric stuff
            science.z = data.z
            science.zquality = data.zquality
            science.zspec = data.z
            science.zcat = data.z
            science.b = data.b_auto
            science.v = data.v_auto
            science.r = data.r_auto
            science.i = data.i_auto
            science.j = data.j_auto
            science.k = data.k_auto
            science.f814w = data.f814w_auto
            science.fuv = data.fuv_auto
            science.nuv = data.nuv_auto
            science.berr = data.b_auto_err
            science.verr = data.v_auto_err
            science.rerr = data.r_auto_err
            science.ierr = data.i_auto_err
            science.jerr = data.j_auto_err
            science.kerr = data.k_auto_err
            science.f814werr = data.f814w_auto_err
            science.fuverr = data.fuv_auto_err
            science.nuverr = data.nuv_auto_err
            science.logmstar = data.mass_kcorrect
        ...
        self.i = 0
        wgood = where(wgood eq 1 and scienceall.oiiew gt -5. and scienceall.nuv gt 50. , cgood)
        scienceall = scienceall[wgood]
        ptr_free, self.science
        self.science = ptr_new(scienceall)
        self.nspec = cgood
...
;    windices = widget_button(windicesbase, value='Compute Indices', uvalue='indices', uname='indices', tab_mode=1, xsize=100)
    wfitomg = widget_button(windicesbase,value='Fit N Mg',uvalue='fitomg',uname='fitomg',tab_mode=1,xsize=100)
