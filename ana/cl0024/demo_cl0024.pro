...
;   good = where(sci.oiiew lt 5. and (sci.fuv_v_rest gt 3. or sci.nuv_mag eq -99.) and sciold.snfit gt 6.83 and sci.logmstar gt 6.,cgood) ;equivalent to sn=10 per ang in rest frame
   ...
   device, filename = psname,xsize = 18,ysize = 8, $
      ...
      !p.charsize=1
      xyouts,5,80,'Cl0024',charsize=1.2

      sunsym = sunsymbol()
      plothist,sci.logmstar,xrange=[9,12],xtitle='log[M!L*!N/M'+sunsym+'!N]',/fill,fcolor=fsc_color('gray'),yrange=[0,90],bin=0.3,position=[0.57,0.15,0.95,0.95],xtickformat='(A1)'
