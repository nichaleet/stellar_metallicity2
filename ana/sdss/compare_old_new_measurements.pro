...
;compare Fe only to MgFe 
   mgfe = mrdfits('/scr2/nichal/workspace4/sps_fit/data/gallazzi_allmass2/sps_fit03.fits.gz',1)
   feonly = mrdfits('/scr2/nichal/workspace2/ana/cl0024/newana_after_referee_report/sci_sdss_ana_afterrev.fits',1)
   ;match data
   match2,mgfe.objname,feonly.objname,subm,subf
   good = where(subm ne -1,cgal)
   mgfe = mgfe(good)
   feonly = feonly(subm(good))

   if total(mgfe.fiber-feonly.fiber) ne 0 then stop,'matching problem. Stopped.'
   ...
   psname = 'plots/sdss_compare_feonly_mgfe_mass_feh.eps'
...
;      cgerrplot,sci(goodfit).logmstar,sci(goodfit).fehlower,sci(goodfit).fehupper,color='tomato',thick=0.5
   ...
   psname = 'plots/sdss_compare_feonly_mgfe_delta_hist.eps'
...
;compare testmodel to MgFe fiducial model
   namefiducial = 'MgNFe'
   fiducial = mrdfits('/scr2/nichal/workspace4/sps_fit/data/gallazzi_allmass2/sps_fit03.fits.gz',1)
   testmodel = mrdfits(testfile,1)

   if total(fiducial.fiber-testmodel.fiber) ne 0 then stop,'matching problem. Stopped.'

   good = where(fiducial.haew gt -1 and fiducial.snfit gt 6.83 and $
   ...
   psname = 'plots/sdss_compare_'+namefiducial+'_'+nametest+'_mass_feh.eps'
   deltasym = '!9' + String("104B) + '!X'
   deltasym = '!9' + String("104B) + '!X'
...
;      cgerrplot,sci(goodfit).logmstar,sci(goodfit).fehlower,sci(goodfit).fehupper,color='tomato',thick=0.5
   ...
   psname = 'plots/sdss_compare_'+nametest+'_'+namefiducial+'_delta_hist.eps'
   ;since it's a histogram, we don't need uncertainty. 
   ;we'll just simple difference (not montecarlo)
   !p.multi = [0,3,2]
...
pro compare_old_new_measurements
   compare_feonly_mgfe
   compare_fiducial_testmodel,'Mgonly','/scr2/nichal/workspace4/sps_fit/data/gallazzi_allmass2/sps_fit01.fits.gz'
end
