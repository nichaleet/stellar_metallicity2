pro mkfake_spec_walpha_nonoise,notput=notput
   ...
   nfinalspec = n_Elements(zmet)*n_Elements(age)*n_Elements(alpha)*nspec
   ...
   for iz=0,n_elements(zmet)-1 do for ia=0,n_elements(age)-1 do for iap=0,n_elements(alpha)-1 do begin
      ...
      for ispec=0,nspec-1 do begin
         ...
         outstr = {lambda:lambdaobs,contdiv:contdiv,contdivivar:contdivivar,dlam:dlam,z:redshift,zmet:zmet(iz),age:age(ia),sn:-99,element:element,element_abund:alpha(iap)}
   
         outname = dir+prefix+zmetname(iz)+agename(ia)+alphaname(iap)+snname+idname(ispec)+'.fits'
