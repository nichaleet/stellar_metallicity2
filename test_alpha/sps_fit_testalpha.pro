...
pro sps_iterproc_alpha, funcname, p, iter, fnorm, functargs=functargs, parinfo=pi, quiet=quiet, dof=dof
    common sps_iterproc, contiter
    common toprint, agediff, zdiff
...
;    endif else pi[2].limits = [40.,400.]; [30.,200.]
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    openw,1,'/scr2/nichal/workspace4/test_alpha/logsps/sps_fit_testalpha'+copynum+'.log',/append
    printf,1, '* * * * * * * * * * * * * * * * * * * *'
    printf,1,systime()
...
pro sps_fit::fitalphaband,science
    widget_control, widget_info(self.base, find_by_uname='status'), set_value='Fitting Mg...'
    znow = science.zfit
    reallambda = science.lambda
    nlambda = n_elements(reallambda)
    ...
    won = where(science.fitmask eq 1 and finite(science.contdiv) and finite(science.contdivivar) $
                and science.contdivivar gt 0 and reallambda/(1.+znow) gt 4895. and $
                reallambda/(1.+znow) lt 5301., con)
    if con lt 10 then begin
       science.responsefn = -999d
...
pro sps_fit::initialize_directory, directory=directory
    ...
    newdirectory = '/scr2/nichal/workspace4/test_alpha/datasps/'+mask_in+'/'
    ...
    ;countfiles:
    sciencefits = self.directory+'sps_fit'+copynum+'.fits.gz'
    files = file_search(directory+'mockms0451*.{fits,fits.gz}', count=c)
...
function sps_fit::INIT, directory=directory, lowsn=lowsn
    ...
    wfitz = widget_button(wprepbase, value='Fit Alpha', uvalue='fitalpha', uname='fitalpha', tab_mode=1, xsize=85)
    ...
    widbase = widget_base(wcurobj, /align_left, /column, xsize=250)
    widlabel = widget_label(widbase, value='object ID:', /align_left, uname='idlabel', xsize=65)
    wcurid = widget_label(widbase, value='     ', /align_left, uname='curid', uvalue='curid', xsize=250)
...
pro sps_fit_testalpha,copyi=copyi,all=all
    common mask_in, mask_in,copynum
    mask = 'mockdata_alpha'
    mask_in = mask
    ...
    directory = '/scr2/nichal/workspace4/test_alpha/'+mask+'/'
